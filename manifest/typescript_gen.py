...
def generate_compact_instructions(
    ...
    lines = [
        ...
        '  """)',
        "",
        "  # Tool inspection (get schema):",
        "  mcproxy(action='execute', code=\"schema = api.server('s').tool.inspect()\")",
        "",
        ...
        "",
        "=== SEARCH (optional) ===",
        "  mcproxy(action='search', query='...')  # Find tools by query",
        "  mcproxy(action='search')                # List all servers/counts",
        "",
        "=== INSPECT ===",
        "  mcproxy(action='inspect', server='name')             # All tools for server",
        "  mcproxy(action='inspect', server='name', tool='foo') # Specific tool schema",
        "",
