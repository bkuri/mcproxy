...
def generate_compact_instructions(
    ...
    lines = [
        ...
        "",
        "MCP Tools (actual names from tools/list, client may add prefix):",
        "  search    - Discover servers/tools (empty = summary)",
        "  execute   - Python code with immediate tool results",
        "",
        "=== EXECUTE ===",
        "  # Response types (auto-unwrapped, no content[0].text needed):",
        "  # - String: file contents, error messages",
        "  # - List: files, entities, collections",
        "  # - Dict: structured data (search results, etc)",
        "",
        ...
        '  """)',
        "",
        "  # Tool inspection (get schema without calling):",
        "  execute(code=\"schema = api.server('s').tool.inspect()\")",
        "  # Returns: {'name': 'tool', 'description': '...', 'inputSchema': {...}}",
        "",
        "=== SEARCH ===",
        f"  search() or search(query='{min_words_display} words')",
